...
- (IBAction)searchsite:(id)sender {
    ...
    switch (siteitem.tag) {
        ...
        case 4: {
            if (_type == MALAnime) {
                openurl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@", @"https://www.reddit.com/search?q=subreddit%3Aanime%20title%3A", [Utility urlEncodeString:_infoviewtitle.stringValue], @"%20episode%20discussion&sort=new"]];
            }
            else {
                openurl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@", @"https://www.reddit.com/search?q=subreddit%3Amanga%20title%3A%20", [Utility urlEncodeString:_infoviewtitle.stringValue], @"%20ch&sort=new"]];
            }
            break;
        }
        case 5: {
        ...
        case 10: {
            switch ([listservice.sharedInstance getCurrentServiceID]) {
                case 1:
                    openurl = [NSURL URLWithString:[NSString stringWithFormat:@"https://aniblogtracker.app/manime.php?aid=%i", self.getSelectedId]];
                    break;
                case 2: {
                    int mappedid = ((NSNumber *)[TitleIDMapper.sharedInstance retrieveTitleIdForService:2 withTitleId:@(self.getSelectedId).stringValue withTargetServiceId:1 withType:0][@"mal_id"]).intValue;
                    openurl = [NSURL URLWithString:[NSString stringWithFormat:@"https://aniblogtracker.app/manime.php?aid=%i", mappedid]];
                    break;
                }
                case 3:
                    openurl = [NSURL URLWithString:[NSString stringWithFormat:@"https://aniblogtracker.app/aanime.php?aid=%i", self.getSelectedId]];
                    break;
                default:
                    break;
            }
